	...
	class MainDAO extends BaseDAO{
		...
		function refreshPubChat(){
			...
			$this->openCon();
				...
				$sql = "SELECT pcm.pcmID,pp.pic_name,pcm.message,ui.Fname,ui.Lname
		...
		function getUsersProfile_Pic($UserID){
			...
			$this->openCon();
				...
				$sql = "SELECT pic_name FROM profile_pics AS pp, users as u WHERE pp.usersID = u.usersID AND pp.usersID=?";
				$stmt = $this->dbCon->prepare($sql);
				$stmt->bindParam(1,$UserID);
				$stmt->execute();
